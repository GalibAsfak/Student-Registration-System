...
root.geometry("1200x700+110+100")
...
def registration_no():
    ...
    max_row_value = sheet.cell(row, column=1).value
...
def Save():
    ...
    try:
        G1 = gender  # if you haven't gender then it will show error message
    except:
    ...
    if N1 == "" or C1 == "Select Class" or D2 == "" or Re1 == "" or S1 == "" or fathername == "" or mothername == "" or F1 == "" or M1 == "":
        ...
        messagebox.showerror("info","Successfully data entered!!")
...
def search():
    text = Search.get() #taking input from box
    clear()  # to clear the data already available in entry box and other
    saveButton.config( state="disable")  #after clicking on search, save  button disable so that no can click on it
    ...
    img=(Image.open("Student Images/"+str(x1)+".jpg")) # for take image name same as registration number
    resized_image=img.resize((190,190))
    photo2=ImageTk.PhotoImage(resized_image)
    lbl.config(image=photo2)
    lbl.image=photo2
...
def Update():
    ...
    # sheet.cell(column=1, row=int(reg_number), value=R1)  now one can update registration no.
    ...
    clear()  # after update it will clear entry boxes
...
Label(root, text="Email: galibasfak73@gmail.com", width=10, height=3, bg="#f0687c", anchor="e").pack(side=TOP, fill=X)
...
Srch = Button(root, text="Search", compound=LEFT, image=imageicon3, width=115, bg="#68ddfa", font="arial 13 bold",command=search)
...
Label(root, text="Registration :", font="arial 13", fg=framebg, bg=background).place(x=30, y=150)
Label(root, text="Date :", font=" arial 13", fg=framebg, bg=background).place(x=500, y=150)

Registration =IntVar()
Date = StringVar()

reg_entry = Entry(root, textvariable=Registration, width=20, font="arial 10")
reg_entry.place(x=135, y=152)
...
obj = LabelFrame(root, text="Student's Details", font=20, bd=2, width=900, bg=framebg, fg="black", height=220,
                 relief=GROOVE)
...
Skill = StringVar()
skill_entry = Entry(obj, textvariable=Skill, width=30, font="arial 10")
...
obj2 = LabelFrame(root, text="Parent's Details", font=20, bd=2, width=900, bg=framebg, fg="black", height=220,
                  relief=GROOVE)
...
Button(root, text="Upload", width=19, height=2, font="arial 12 bold", bg="lightblue", command=showimage).place(x=970,
                                                                                                               y=370)

saveButton = Button(root, text="Save", width=19, height=2, font="arial 12 bold", bg="lightgreen", command=Save)
saveButton.place(x=970, y=450)
Button(root, text="Reset", width=19, height=2, font="arial 12 bold", bg="lightpink", command=clear).place(x=970, y=530)

Button(root, text="Exit", width=19, height=2, font="arial 12 bold", bg="grey", command=Exit).place(x=970, y=610)
